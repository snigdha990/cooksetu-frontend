  ...
  const filtered = cooks.filter((c) => {
    const matchesLocation =
      locationAllowed || manualLocation === ""
        ? true
        : c.locationString?.toLowerCase().includes(manualLocation.toLowerCase());
    const matchesCuisine = cuisine === "" || c.cuisines.includes(cuisine);
  ...
  return (
    <div className="min-h-screen px-6 py-28 bg-gradient-to-br from-indigo-950 via-purple-950 to-violet-950">
      <div className="max-w-7xl mx-auto">
        ...
        <p className="text-white/70 mb-8">Hire trusted home cooks based on your location & cuisine</p>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {loading
            ? Array.from({ length: 6 }).map((_, i) => <CookCardSkeleton key={i} />)
            : filtered.map((cook) => (
                <div
                  key={cook._id}
